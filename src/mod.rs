...
fn build_models_help_response(current: &ChannelRouteSelection, workspace_dir: &Path) -> String {
    ...
    response.push_str("\n使用 `/model <模型ID>` 切换模型。\n");
    response.push_str("使用 `/approve-request <工具名>` 请求工具授权。\n");
    response.push_str("使用 `/approve-all-once` 请求一次性全工具授权。\n");
    response.push_str("使用 `/approve-confirm <请求ID>` 确认授权。\n");
    response.push_str("使用 `/approve-deny <请求ID>` 拒绝授权。\n");
    response.push_str("使用 `/approve-pending` 查看待处理请求。\n");
    response.push_str("使用 `/approve <工具名>` 授权工具。\n");
    response.push_str("使用 `/unapprove <工具名>` 撤销授权。\n");
...
async fn handle_runtime_command_if_needed(
    ...
    let response = match command {
        ...
        ChannelRuntimeCommand::ConfirmToolApproval(raw_request_id) => {
            let request_id = raw_request_id.trim().to_string();
            if request_id.is_empty() {
                "用法: `/approve-confirm <请求ID>`".to_string()
            } else {
                match ctx.approval_manager.confirm_non_cli_pending_request(
                    &request_id,
                    sender,
        ...
        ChannelRuntimeCommand::ListPendingApprovals => {
            ...
            if rows.is_empty() {
                ...
                let mut response = String::new();
                response.push_str("待处理的授权请求 (sender+chat/channel 范围):\n");
                for req in rows {
                    let reason = req
                        .reason
                        .as_deref()
