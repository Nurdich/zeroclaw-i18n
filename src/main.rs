...
#[command(version)]
#[command(about = "最小最快的人工智能助手。", long_about = None)]
struct Cli {
...
enum Commands {
    ...
    Onboard {
        ...
        /// API 密钥（快速模式使用，--interactive 时忽略）
        #[arg(long)]
        api_key: Option<String>,

        /// 提供商名称（快速模式使用，默认：openrouter）
        #[arg(long)]
        provider: Option<String>,
        /// 模型 ID 覆盖（快速模式使用）
        #[arg(long)]
        model: Option<String>,
        /// 记忆后端（sqlite, lucid, markdown, none）- 快速模式使用，默认：sqlite
        #[arg(long)]
...
Examples:
  ...
  zeroclaw agent -m \"quick task\" --memory-backend none --compact-context")]
    Agent {
        ...
        /// 附加外设（开发板:路径，例如 nucleo-f401re:/dev/ttyACM0）
        #[arg(long)]
...
Examples:
  ...
  zeroclaw gateway --new-pairing    # clear tokens and generate fresh pairing code")]
    Gateway {
        /// 监听端口（使用 0 随机分配）；默认为配置 gateway.port
        #[arg(short, long)]
        port: Option<u16>,

        /// 绑定主机；默认为配置 gateway.host
        #[arg(long)]
        host: Option<String>,
    ...
    /// 启动长时间运行的自主运行时（gateway + channels + heartbeat + scheduler）
    #[command(long_about = "\
...
Examples:
  ...
  zeroclaw daemon --host 127.0.0.1  # localhost only")]
    ...
    Service {
        /// 使用的初始化系统：auto（检测）、systemd 或 openrc
    ...
    /// 运行 daemon/scheduler/channel 状态诊断
    Doctor {
        #[command(subcommand)]
    ...
    /// 将 ZeroClaw 更新到最新版本
    #[command(long_about = "\
从 GitHub 将 ZeroClaw 更新到最新版本。

下载适合您平台的可执行二进制文件并替换当前的可执行文件。需要对二进制文件位置有写入权限。

Examples:
  zeroclaw update              # Update to latest version
  zeroclaw update --check      # 检查更新而不安装
...
enum ModelCommands {
    ...
    Refresh {
        /// 提供商名称 (defaults to configured default provider)
        #[arg(long)]
        provider: Option<String>,
