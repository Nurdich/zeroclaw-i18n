...
pub async fn run_wizard(force: bool) -> Result<Config> {
    ...
    println!(
        "  {}",
        style("欢迎使用 ZeroClaw — 最小最快的人工智能助手。")
            .white()
            .bold()
    );
    println!(
        "  {}",
        style("此向导将在 60 秒内配置您的 agent。").dim()
    );
    println!();

    print_step(1, 10, "工作区设置");
    ...
    print_step(2, 10, "AI 提供商和 API 密钥");
    let (provider, api_key, model, provider_api_url) = setup_provider(&workspace_dir).await?;

    print_step(3, 10, "频道（您如何与 ZeroClaw 交流）");
    let channels_config = setup_channels()?;

    print_step(4, 10, "隧道（暴露到互联网）");
...
pub async fn run_channels_repair_wizard() -> Result<Config> {
    ...
    print_step(1, 1, "频道（您如何与 ZeroClaw 交流）");
...
async fn run_quick_setup_with_home(
    ...
    if credential_override.is_none() {
        if provider_supports_keyless_local_usage(&provider_name) {
            println!("    1. Chat:     zeroclaw agent -m \"你好！\"");
            ...
            if canonical_provider_name(&provider_name) == "copilot" {
                ...
                println!("    2. Chat:              zeroclaw agent -m \"你好！\"");
            ...
            println!("    3. Chat:              zeroclaw agent -m \"你好！\"");
...
fn print_summary(config: &Config) {
    ...
    println!(
        "       {}",
        style("zeroclaw agent -m \"你好，ZeroClaw！\"").yellow()
    );
    println!();
    step += 1;
